...
[z, lambda] = fmincon(f,z0,[],[],Aeq,beq,vlb,vub,@e_constraint, options);
t1=toc;
...
function [c,ceq] = e_constraint(x)
    ...
    c = zeros(N,1);
    for i=1:N
        c(i) = alpha_*exp(-beta_*(x((1+(i-1)*nx))-lambda_t_)^2)-x(5+(i-1)*nx);
    end
    ceq = [];
end
